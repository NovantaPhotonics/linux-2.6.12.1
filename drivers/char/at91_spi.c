...
#define CSR0_OFFSET 12
static AT91_REG* csr = (AT91_REG*)(AT91C_VA_BASE_SPI + CSR0_OFFSET*sizeof(AT91_REG));
...
void spi_access_bus(short device)
{
	...
	if (device==3)
	{
	    //printk("SPI Local Loopback enabled\n");
...
int spi_transfer(struct spi_transfer_list* list)
{
	...
	/*If we are in 16 bit SPI mode, we have to divide by two what we pass to the PDC. */
	int div = (csr[current_device] & AT91C_SPI_BITS_16) ? 2 : 1;	
	/* Store transfer list */
	device->xfers = list;
	list->curr = 0;
	...
	controller->SPI_TCR = list->txlen[0]/div;
	controller->SPI_RCR = list->rxlen[0]/div;
	...
	if (list->nr_transfers > 1) {
		...
		controller->SPI_TNCR = list->txlen[1]/div;
		controller->SPI_RNCR = list->rxlen[1]/div;
	}
	else {
		device->txnext = 0;
		device->rxnext = 0;
...
//	if (div == 2)
	{
	    uint* pbuf = (uint*)(list->rx[list->curr]);
	    list->servo_rxval = (uint)(pbuf[list->rxlen[list->curr]/4 - 1]);
	}
	...
	if (list->curr == list->nr_transfers) {		/* all transfers complete */
		...
		controller->SPI_TNPR = device->txnext;
		controller->SPI_RNPR = device->rxnext;
		controller->SPI_TNCR = list->txlen[i]/div;
		controller->SPI_RNCR = list->rxlen[i]/div;
	}
	return IRQ_HANDLED;
}
...
static int __init at91_spi_init(void)
{
	...
	controller->SPI_CSR0 = (1 << 24) | AT91C_SPI_CPOL | AT91C_SPI_BITS_8 | (16 << 16) | (DEFAULT_SPI_BAUD << 8);
	controller->SPI_CSR1 = (1 << 24) | AT91C_SPI_CPOL | AT91C_SPI_BITS_8 | (16 << 16) | (DEFAULT_SPI_BAUD << 8);
