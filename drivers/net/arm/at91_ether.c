...
{
	void __iomem *emac_base = (void __iomem *)AT91C_VA_BASE_EMAC;
...
static inline void at91_phy_wait(void) 
{
	...
	while (!(at91_emac_read(AT91_EMAC_SR) & AT91C_EMAC_IDLE)) {
...
static int update_linkspeed(struct net_device *dev, AT91PS_EMAC regs) {
	...
	//printk(KERN_INFO "Testing sbs driver");
...
static int at91ether_ethtool_ioctl (struct net_device *dev, void *useraddr)
{
	...
	case ETHTOOL_SRINGPARAM: { /*This is used to turn the Green light on and off accordingly*/
		...
		spin_lock_irq(&lp->lock);
		enable_mdi(regs);
		u16 value = edata.data & 0xff ? 0x8782/*Green:On*/ : 0x9782/*Green:Off*/ ;
		/*yellow: activity, Green: speed (On/100 Off/10), LED3:On*/
		write_phy(regs, lp->phy_address, MII_LEDCTRL_REG, value);
...
static int __init at91ether_setup(unsigned long phy_type, unsigned short phy_address, struct at91_eth_data *board_data)
{
	...
	{
		/*yellow: activity, Green: speed (On/100 Off/10), LED3:On*/
		write_phy(regs, phy_address, MII_LEDCTRL_REG, 0x9782);
	}
	if (phy_type == MII_DM9161_ID) {
		...
		write_phy(regs, phy_address, MII_LEDCTRL_REG, 0x0b22);
	}
	disable_mdi(regs);
	spin_unlock_irq(&lp->lock);
...
static int __init at91ether_probe(struct device *dev)
{
	...
	while ((detected != 0) && (phy_address < 32)) {
		...
		//check to see if it is an LXT972A or greater, if it is, set it to a LXT971A
		if ( (phy_id >> 4) == (MII_LXT971A_ID >> 4))
		{
		    phy_id = MII_LXT971A_ID;
                }
		switch (phy_id) {
			case MII_DM9161_ID:		/* Davicom 9161: PHY_ID1 = 0x181, PHY_ID2 = B881 */
			case MII_LXT971A_ID:		/* Intel LXT971A or greater: PHY_ID1 = 0x13, PHY_ID2 = 78Ex */
