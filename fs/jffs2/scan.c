...
scan_more:	
	while(ofs < jeb->offset + c->sector_size) {
		...
		if (ofs + je32_to_cpu(node->totlen) > 
			...
			printk(KERN_WARNING "Node at 0x%08x with length 0x%08x would run over the end of the erase block(offset:0x%x , size:0x%x)\n",
			       ofs, je32_to_cpu(node->totlen),jeb->offset,c->sector_size);
